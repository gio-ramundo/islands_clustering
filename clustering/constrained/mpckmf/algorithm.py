...
for w in weights:
    ...
    for n_clust in range(5,16):
        ...
        mpckmf.fit(df[relevant_col].values, cl = cl)
        score_mpckmf = silhouette_score(df[relevant_col], mpckmf.labels_)
        shilouette.append(score_mpckmf)
        df[f'cluster_label_mpckmf_{n_clust}_{w}'] = mpckmf.labels_
        # Violations list update
        violation = violations_counter(mpckmf.labels_)
        if w == 0.5:
    ...
    plt.ylabel(f'Shilouette score MPCKMeansMF')
    plt.grid(True)
