...
for k, (i, isl) in enumerate(gdf.iterrows(), 1):
    if (k-1)%10 == 0:
        ...
        # Not valid values mask and geomtry extraction
        ...
        area1 = ee_geometry_valid_land.area().getInfo()
        gdf.loc[i,'land'] = area1/area0
    else:
        gdf = gdf.drop(i)

print(f'Mean area ratio {np.mean(gdf[f'land'])}')